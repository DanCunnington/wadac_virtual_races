...
module.exports = (app, db, strava) => {
	...
	app.post('/submit_result', (req, res) => {
		...
		let athlete_email_consent = new_result.athlete_email_consent
		...
		if (!event_id || !activity_name || !activity_type || !athlete_email || !athlete_email_consent || !activity_id || !start_date || !athlete_name || 
			!elapsed_time || !moving_time || ! elevation_gain || !distance) {
			res.status(400)
			return res.json({"err": "please specify event_id, athlete_name, activity_type, athlete_email, athlete_email_consent, activity_id, "+
		...
		let insertResultToDb = function(net_elevation_change) {
			// Insert to database			
			let result = {event_id, athlete_name, athlete_email, athlete_email_consent, activity_id, activity_name, 
